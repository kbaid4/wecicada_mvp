...
const SupplierHomepage = () => {
  ...
  const createTestNotification = async (email) => {
    ...
    try {
      ...
      console.log('DIRECT TEST: Creating notification directly with auth user:', normalizedEmail);
      
      const testNotification = {
        supplier_email: normalizedEmail,
        event_id: 'test-event-id-' + Math.floor(Math.random() * 100000),
      ...
      if (insertError) {
        ...
        console.log('DIRECT TEST: Successfully created test notification:', result);
        console.log('If this succeeds but notifications still don\'t show, the issue is with retrieval, not creation');
      }
    } catch (err) {
